...
def get_date_rate(code_list):
    ...
    for code in code_list:
        ...
        url = "http://www.chinabond.com.cn/jsp/include/EJB/fxdf.jsp?sZqdm=%s"%code
        html = download(url)
        goal_list=[]
        try:
            goal_list = re.findall("<tr>\s+<td\s+width=10%\s+nowrap>[1-9][0-9]{0,2}</td>\s+<td\s+width=25%\s+nowrap>(.*?)</td>\s+<td\s+width=25%\s+nowrap>(.*?)</td>\s+<td\s+width=15%\s+nowrap>(.*?)</td>",html)
            goal_list = [goal for goal in goal_list if not goal[2] == "0"]
            #goal_list = re.findall("<tr>\s+<td\s+width=10%\s+nowrap>[1-9][0-9]{0,2}</td>\s+<td\s+width=25%\s+nowrap>(.*?)</td>\s+<td\s+width=25%\s+nowrap>(.*?)</td>\s+<td\s+width=15%\s+nowrap>[1-9]\d*(\.\d*[1-9])?)|(0\.\d*[1-9]</td>",html)
