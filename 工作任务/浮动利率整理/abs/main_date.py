...
def excel_intersection (masterTable,slaveTable,newTable,begin_index = 0, by_index=0):
    ...
    ws1 = wb.add_sheet('sheet1');#新建新表标签
    ...
    for colnum in range(0, ncols):#在新表中写入头标题，并加粗
        ws1.write(0, colnum, headData[colnum], xlwt.easyxf('font: bold on'))
        ...
        while(count >= 0):
            for colnum in range(0, ncols):
                if table.cell(rownum, colnum).ctype == 3:  #如果是日期类型，进行datetime转换(只转换一次)
                    ...
                    ws1.write(index, colnum, temp)
                else:
                    ws1.write(index, colnum, rowData[colnum])
            count -= 1
            index += 1
    table = masterData.sheets()[by_index+1]
    nrows = table.nrows#行数
    ncols = table.ncols#列数 
    index = 0
    for rownum in range(0,nrows):#第二行开始遍历
        rowData = table.row_values(rownum)
        for colnum in range(0,ncols):
            ws2.write(index, colnum, rowData[colnum])
        index+=1
...
#主函数入口
def main():
    excel_intersection("ABS-1-3-5load.xls","ABS-1-3-5load.xls","ABS-1-3-5load-output.xls",0, 0)
    excel_intersection("ABS-1-load.xls","ABS-1-load.xls","ABS-1-load-output.xls",0, 0)
    excel_intersection("ABS-1-deposit.xls","ABS-1-deposit.xls","ABS-1-deposit-output.xls",0, 0)
    excel_intersection("ABS-5m-load.xls","ABS-5m-load.xls","ABS-5m-load-output.xls",0, 0)
